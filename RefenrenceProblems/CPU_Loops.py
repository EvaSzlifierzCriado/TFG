...
def matrixConv3x3(M, conv):
    ...
    for i in range(1, len(M)-1):
        for j in range(1, len(M[0])-1):
            res[i][j] = M[i-1][j-1] * conv[0][0] + M[i-1][j] * conv[0][1] + M[i-1][j+1] * conv[0][2] + M[i][j-1] * conv[1][0] + M[i][j] * conv[1][1] + M[i][j+1] * conv[1][2] + M[i+1][j-1] * conv[2][0] + M[i+1][j] * conv[2][1] + M[i+1][j+1] * conv[2][2]
    return res
